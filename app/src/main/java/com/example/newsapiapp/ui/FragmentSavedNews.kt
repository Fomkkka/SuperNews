...
class FragmentSavedNews : Fragment(), MenuProvider {
    ...
    override fun onViewCreated(view: View, savedInstanceState: Bundle?) {
        ...
        (activity as AppCompatActivity).supportActionBar?.setTitle("Збережене")
    ...
    override fun onMenuItemSelected(menuItem: MenuItem): Boolean {
        ...
        if (menuItem.itemId == R.id.deleteAll){
            ...
            builder.setMessage("Видалити збережені новини?")

            builder.setPositiveButton("Видалити все"){dialog, which->


                viewModel.deleteAll()
                Toast.makeText(context, "Видалити все", Toast.LENGTH_SHORT).show()

                view?.findNavController()?.navigate(R.id.action_fragmentSavedNews_to_fragmentBreakingNews)

            }

            builder.setNegativeButton("Відміна"){dialog, which->
